...
public class SecretRequestAdvice extends RequestBodyAdviceAdapter {
    ...
    /**
           * 解密请求体，3des解析（cbc模式）
     *
     * @param inputMessage 请求体
     * @return 明文
     */
    private String decryptBody(HttpInputMessage inputMessage) throws IOException {
        InputStream encryptStream = inputMessage.getBody();
        String encryptBody = StreamUtils.copyToString(encryptStream, Charset.defaultCharset());
        logger.info("original request body: {}", encryptBody);
        String result = DesCbcUtil.decode(encryptBody, secretProperties.getDesSecretKey(), secretProperties.getDesIv());
        logger.info("decoded request body: {}", result);
        return result;
    }
